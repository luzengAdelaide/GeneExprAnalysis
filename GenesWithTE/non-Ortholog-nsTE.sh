...
do
    for j in "${arr2[@]}"
    do
	echo nonortho_bed_$i bed.$j.$i
	intersectBed -a nonortho_bed_$i -b bed.$j.$i > overlap_all_$i\_$j
    done
done
...
do
    ...
    intersectBed -a large_nonortho_bed_mdo -b large_bed.$i.mdo > large.overlap_all_mdo_$i
    intersectBed -a small_nonortho_bed_mdo -b small_bed.$i.mdo > small.overlap_all_mdo_$i
...
do
    for j in "${arr2[@]}"
    do
	awk '{print $4}' uniq.overlap_all_$i\_$j > all_$i\_$j\_nonortho
    done
done
...
do
    for j in "${arr2[@]}"
    do
    perl ../../remove.same.pl recent_$i\_$j\_nonortho all_$i\_$j\_nonortho > noRTE_nonortho_$i\_$j
    done
done
