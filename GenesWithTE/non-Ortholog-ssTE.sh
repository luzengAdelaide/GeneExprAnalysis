...
do
    for j in "${arr2[@]}"
    do
	echo nonortho_bed_$i $i\_recent_$j.use
	intersectBed -a nonortho_bed_$i -b $i\_recent_$j.use > overlap_recent_$i\_$j
    done
done
...
do
    ...
    intersectBed -a large_nonortho_bed_mdo -b large_mdo_recent_$i.use > large.overlap_recent_mdo_$i
    intersectBed -a small_nonortho_bed_mdo -b small_mdo_recent_$i.use > small.overlap_recent_mdo_$i
    cat large.overlap_recent_mdo_$i small.overlap_recent_mdo_$i > overlap_recent_mdo_$i
done

for i in overlap_recent_*
do
...
do
    for j in "${arr2[@]}"
    do
	awk '{print $4}' uniq.overlap_recent_$i\_$j > recent_$i\_$j\_nonortho
    done
done
